...
################################################################################
#
# Python implementation of the usb_rawhid host-side functions for the Teensy
# (and other raw HID devices). Requires the libhid library.
#
# Craig Heffner
# 21-July-2011
...
class RawHID:
	...
	def open(self, vid, pid):
		...
		if hid_ret == HID_RET_SUCCESS:
            ...
            hid_ret = hid_force_open(self.hid, self.INTERFACE, match,
			...
			if hid_ret == HID_RET_SUCCESS:
                ...
                sys.stderr.write(
                    "hid_force_open() failed with error code: %d\n" % hid_ret)
		elif self.verbose:
            sys.stderr.write(
                "hid_init() failed with error code: %d\n" % hid_ret)
